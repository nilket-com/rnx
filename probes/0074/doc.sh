#!/usr/bin/env bash
# Record 0074: rustdoc JSON for the polars facade at a pinned git commit,
# the adapter's feature set, with the 0072 extractor's crate selection.
#
#   doc.sh <rev> <label>
#
...
host="$root/target/0074/host-$cfg"
out="$root/probes/0074/out/$cfg"
lock="$here/locks/doc-$cfg.lock"
...
FULL_OMIT=""
case "$cfg" in
  rc2) features='default-features = false, features = ["lazy", "csv", "parquet"]' ;;
  full)
...
[workspace]
[dependencies]
polars = { git = "$GIT_URL", rev = "$release", $features }
TOML
echo '' > "$host/src/lib.rs"
export CARGO_TARGET_DIR="$root/target/0074/target-$cfg"
...
json.dump({"git":"https://github.com/pola-rs/polars","rev":release,"cfg":cfg,"toolchain":nightly,"rustc":sh(f"rustc +{nightly} --version"),
  ...
  "lock":f"probes/0074/locks/doc-{cfg}.lock"},

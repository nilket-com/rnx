...
of scope), and `plans/0073_polars_generator_evidence.md` gives the counts
and the executed coverage. Building with `--no-default-features` gives the
...
- Types: every concrete reachable Polars type a binding mentions has a
  ...
  wins and the entry is listed as adapted.

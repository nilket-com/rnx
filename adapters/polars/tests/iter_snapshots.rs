//! Record 0103: `downcast_iter()` on the fourteen wrappers, driven into owned nested vectors in chunk order, compared with direct Polars and bounded as a whole.
...
/// The script's rendering of a direct `downcast_iter`.
macro_rules! rust_show {
    ($ca:expr, $cell:expr) => {{
        let parts: Vec<String> = $ca.downcast_iter().map(|a| a.iter().map(|x| x.map_or("n".to_string(), $cell)).collect::<Vec<_>>().join(",")).collect();
...
fn script(recv: &str, cell: &str) -> String {
    run(&format!(r#"{SHOW}
        pub fn main() {{
            ...
            let kept = {{ let t = {recv}; t.downcast_iter() }};
            `${{show(r.downcast_iter(), {cell})}} ${{show(e.downcast_iter(), {cell})}} ${{show(kept, {cell})}} ${{show(r.downcast_iter(), {cell})}}`
        }}
    "#))
}
macro_rules! direct {
...
#[test]
fn all_fourteen_owners_iterate_their_chunks_in_order() {
...
fn extremes_floats_and_u64_read_back() {
    ...
    let got = run(&format!(r#"{SHOW}
        pub fn main() {{
            ...
            let failed = show(u.downcast_iter(), b);
            let after = match u.get(0) {{ Ok(Some(v)) => v, _ => 0 }};
            `${{show(i.downcast_iter(), b)}} ${{show(f.downcast_iter(), b)}} ${{show(d.downcast_iter(), b)}} | ${{failed}} | ${{after}}`
        }}
    "#));
    assert_eq!(got, format!("1[-128,127,0] 1[0.10000000149011612,-0.0] 1[-0.0,NaN,inf,-inf] | ConversionError: downcast_iter: 9223372036854775808 does not fit a script integer | {}", i64::MAX));
}

#[test]
fn the_whole_iterator_is_bounded() {
    let _serial = SERIAL.lock().unwrap_or_else(|e| e.into_inner());
    ...
    let got = run(&format!(r#"{SHOW}
        pub fn main() {{
            ...
            let out = [];
            polars::set_materialize_limit(7); out.push(show(r.downcast_iter(), b)); out.push(match r.downcast_get(1) {{ Ok(Some(v)) => `one chunk fits: ${{v.len()}} cells`, _ => "no" }});
            polars::set_materialize_limit(8); out.push(show(r.downcast_iter(), b));
            polars::set_materialize_limit(14); out.push(show(t.downcast_iter(), q));
            polars::set_materialize_limit(15); out.push(show(t.downcast_iter(), q));
            polars::set_materialize_limit(0);
            out.iter().fold("", |acc, x| acc + x + "\n")
        }}
    "#));
    assert_eq!(got.lines().collect::<Vec<_>>(), [
        "MaterializeLimit: downcast_iter: 2 chunks and 6 cells (8 slots), more than the bound of 7",
        "one chunk fits: 3 cells",
        "2[1,2,3|1,n,3]",
        "MaterializeLimit: downcast_iter: 2 chunks, 4 cells and 9 bytes (15 slots), more than the bound of 14",
        "2[\"é日本\",\"\"|n,\"z\"]",

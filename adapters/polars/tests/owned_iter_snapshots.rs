//! Record 0105: `downcast_into_iter()` on the fourteen wrappers consumes a clone: its owned arrays are streamed into owned nested vectors, compared with direct Polars, bounded before the clone, and the receiver stays usable.
...
/// The script's rendering of a direct `clone().downcast_into_iter()`.
macro_rules! rust_show {
    ($ca:expr, $cell:expr) => {{
        let ca = &$ca;
        let parts: Vec<String> = ca.clone().downcast_into_iter().map(|a| a.iter().map(|x| x.map_or("n".to_string(), $cell)).collect::<Vec<_>>().join(",")).collect();
...
fn script(recv: &str, cell: &str) -> String {
    run(&format!(r#"{SHOW}
        pub fn main() {{
            ...
            let kept = {{ let t = {recv}; t.downcast_into_iter() }};
            `${{show(r.downcast_into_iter(), {cell})}} ${{show(e.downcast_into_iter(), {cell})}} ${{show(kept, {cell})}} ${{show(r.downcast_into_iter(), {cell})}}`
        }}
    "#))
}
macro_rules! direct {
...
#[test]
fn all_fourteen_owners_stream_their_owned_chunks() {
...
fn extremes_floats_and_u64_read_back() {
    ...
    let got = run(&format!(r#"{SHOW}
        pub fn main() {{
            ...
            let failed = show(u.downcast_into_iter(), b);
            let after = match u.get(0) {{ Ok(Some(v)) => v, _ => 0 }};
            let still = u.chunk_lengths().unwrap().len();
            `${{show(i.downcast_into_iter(), b)}} ${{show(f.downcast_into_iter(), b)}} ${{show(d.downcast_into_iter(), b)}} | ${{failed}} | ${{after}} ${{still}}`
        }}
    "#));
    assert_eq!(got, format!("1[-128,127,0] 1[0.10000000149011612,-0.0] 1[-0.0,NaN,inf,-inf] | ConversionError: downcast_into_iter: 9223372036854775808 does not fit a script integer | {} 1", i64::MAX));
}

#[test]
fn the_whole_call_is_bounded_before_the_clone() {
    let _serial = SERIAL.lock().unwrap_or_else(|e| e.into_inner());
    ...
    let got = run(&format!(r#"{SHOW}
        pub fn main() {{
            ...
            let out = [];
            polars::set_materialize_limit(7); out.push(show(r.downcast_into_iter(), b)); out.push(match r.downcast_get(1) {{ Ok(Some(v)) => `one chunk fits: ${{v.len()}} cells`, _ => "no" }});
            polars::set_materialize_limit(8); out.push(show(r.downcast_into_iter(), b));
            polars::set_materialize_limit(14); out.push(show(t.downcast_into_iter(), q));
            polars::set_materialize_limit(15); out.push(show(t.downcast_into_iter(), q));
            polars::set_materialize_limit(0);
            out.iter().fold("", |acc, x| acc + x + "\n")
        }}
    "#));
    assert_eq!(got.lines().collect::<Vec<_>>(), [
        "MaterializeLimit: downcast_into_iter: 2 chunks and 6 cells (8 slots), more than the bound of 7",
        "one chunk fits: 3 cells",
        "2[1,2,3|1,n,3]",
        "MaterializeLimit: downcast_into_iter: 2 chunks, 4 cells and 9 bytes (15 slots), more than the bound of 14",
        "2[\"é日本\",\"\"|n,\"z\"]",

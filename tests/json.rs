//! What `json::stringify` does, and what it refuses. Record 0019
...
fn a_cycle_is_refused_rather_than_aborting_the_process() {
	...
	let r = evaluated("let v = [1]; v.push(v); json::stringify(v)");
...
fn repeated_shared_data_is_not_a_cycle() {
	...
	let r = evaluated("let a = [1]; json::stringify([a, a])");
...
fn the_depth_bound_is_shared_with_the_renderer() {
	let inside = evaluated(&format!("{} json::stringify(v)", nested(256)));
	assert_eq!(inside.code, 0, "at the bound: {}", inside.stderr);
	let past = evaluated(&format!("{} json::stringify(v)", nested(257)));
...
fn every_arm_of_the_serializer_is_mirrored() {
	...
	for (source, refusal) in [
		("json::stringify(Ok(1))", "cannot serialize a result"),
		("json::stringify(Err(1))", "cannot serialize a result"),
		("json::stringify(|| 1)", "cannot serialize a function"),
		(
			"fn h() {} json::stringify(h)",
			"cannot serialize a function",
		),
		("json::stringify(0..3)", "an external reference"),
		(
			"struct P { c } json::stringify(P { c: 1 })",
			"cannot serialize struct P",
		),
		(
			"enum E { A } json::stringify(E::A)",
...
fn a_refusal_names_where_it_happened() {
	for (source, path) in [
		(
			"json::stringify(#{a: #{b: #{c: #{d: #{e: #{f: #{g: [1, || 1]}}}}}}})",
			".a.b.c.d.e.f.g[1]",
		),
		("json::stringify([[[|| 1]]])", "[0][0][0]"),
		("json::stringify(|| 1)", "the value itself"),
	] {
		let r = evaluated(source);
...
fn a_path_cannot_be_confused_with_one_that_looks_like_it() {
	...
	let dotted = evaluated("json::stringify(#{\"a.b\": || 1})");
	let nested_keys = evaluated("json::stringify(#{a: #{b: || 1}})");
	assert!(
	...
	let numeric_key = evaluated("json::stringify(#{\"3\": || 1})");
	let index = evaluated("json::stringify([0, 1, 2, || 1])");
	assert!(
	...
	let quoted = evaluated("json::stringify(#{\"say \\\"hi\\\"\": || 1})");
	assert!(
		quoted.stderr.contains("[\"say \\\"hi\\\"\"]"),
		"{}",
		quoted.stderr
	);
	let controlled = evaluated("json::stringify(#{\"a\\u{1b}b\": || 1})");
	assert!(
...
fn the_depth_bound_covers_a_path_with_no_container_on_it() {
	...
	let inside = evaluated(&format!("{} json::stringify(v)", options(256)));
	assert_eq!(inside.code, 0, "at the bound: {}", inside.stderr);
	assert_eq!(inside.stdout, "\"1\"\n");

	let past = evaluated(&format!("{} json::stringify(v)", options(257)));
	assert!(!past.signalled, "the process died on a signal");
	...
	let mixed = evaluated("let v = 1; for i in 0..200 { v = [Some(v)] } json::stringify(v)");
...
fn the_two_walks_agree_on_what_is_too_deep() {
	...
	for deep in [256, 257] {
		...
		let serialized = evaluated(&format!("{} json::stringify(v)", options(deep)));
		assert_eq!(
